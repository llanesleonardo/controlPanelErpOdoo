...
4. **Consume** via `@control-panel-ontology/contracts` from gateway / tools; orchestrator skills key off the same intent codes.

Parent: [ControlPanelOntology_TSD](../../TSD/ControlPanelOntology_TSD.md) - Patterns: [Pattern_Selection](../../TSD/Pattern_Selection.md) - Layout: [Monorepo_Layout](../../TSD/Monorepo_Layout.md).
...
resources/packages/contracts/
  package.json                 # @control-panel-ontology/contracts
  README.md
  src/
    index.js                   # re-exports
...
Workspace name: `@control-panel-ontology/contracts`.
...
# or
npm run sync-schemas -w @control-panel-ontology/contracts
```
...
| Connectors (SAC-005) | Map codes → vendor calls behind ACL on **peer** connectors |

Vendor field names and RPC stay in connectors — not in taxonomy YAML.

## AI + human classify (OPS-020 / SRD-EDGE-007)

| Rule | Design |
|------|--------|
| Input | NL / multimodal suggestion enters `POST /intents/classify` (or equivalent) |
| Output | Known `intent_code` only — or explicit insufficient / unknown |
| Execute | Separate step: human confirm or policy → allowlisted skill (SAC-001 / SAC-004) |
| Forbidden | Inventing tools, connectors, or payloads in the classify path |
...
- [Authoring/](./Authoring/) · [resources/packages/contracts](../../../../resources/packages/contracts/) · [OPS-010](./Scenarios/OPS-010.md) · [OPS-020](./Scenarios/OPS-020.md)
