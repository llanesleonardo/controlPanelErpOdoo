...
|-------|----------------|
| Product / platform | Connector SPI, capability matrix, intent→port mapping, allowlist, evidence envelope |
| Connector owner (us) | Auth, vendor API, field/module maps, health, schema sync, simulate mode |
...
1. **Odoo** — live today (`sales.estimate.read`); prove SPI against real SoR  
...
UI and allowlist only offer skills the tenant’s **enabled connectors** declare.

| Skill | `odoo` | future… |
|-------|--------|---------|
| `sales.estimate.read` | yes | — |
| estimate-issues (Epic-06) | planned | — |
...
All of that stays **inside** the connector package/adapters. Control panel, gateway contracts, and taxonomy stay connector-agnostic.
